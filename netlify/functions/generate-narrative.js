...
function buildExecutiveNarrative(metrics, url) {
  ...
  function findSignal(key) {
    var list = Array.isArray(metrics.delivery_signals) ? metrics.delivery_signals : [];
    var k = String(key || "").toLowerCase();
    for (var i = 0; i < list.length; i++) {
      var s = list[i] || {};
      var id = String(s.id || s.label || "").toLowerCase();
      if (id.indexOf(k) !== -1) return s;
    }
    return null;
  }

  // Decide whether "mobile vs desktop" is actually the story.
  var hasPsi = (mobile.LCP_ms != null || mobile.FCP_ms != null || mobile.TTFB_ms != null || mobile.TBT_ms != null ||
                desktop.LCP_ms != null || desktop.FCP_ms != null || desktop.TTFB_ms != null || desktop.TBT_ms != null);

  var mLCP = mobile.LCP_ms, dLCP = desktop.LCP_ms;
  var mFCP = mobile.FCP_ms, dFCP = desktop.FCP_ms;
  var mTTFB = mobile.TTFB_ms, dTTFB = desktop.TTFB_ms;
  var mTBT = mobile.TBT_ms, dTBT = desktop.TBT_ms;

  var mobileBad = (mLCP != null && mLCP >= 6000) || (mFCP != null && mFCP >= 3000) || (mTTFB != null && mTTFB >= 800) || (mTBT != null && mTBT >= 600);
  var desktopBad = (dLCP != null && dLCP >= 6000) || (dFCP != null && dFCP >= 3000) || (dTTFB != null && dTTFB >= 800) || (dTBT != null && dTBT >= 600);

  var lcpRatio = (mLCP != null && dLCP != null && dLCP > 0) ? (mLCP / dLCP) : null;
  var splitIsStory = (lcpRatio != null && lcpRatio >= 1.6 && (dLCP != null && dLCP < 4000)); // big gap + desktop is OK

  var secSig = findSignal("security");
  var seoSig = findSignal("seo");
  var accessSig = findSignal("access");
  var secMissingCount = secSig && secSig.evidence && typeof secSig.evidence.missing_count === "number" ? secSig.evidence.missing_count : null;

  // 1) Mobile delivery constraint (common, but do NOT force desktop comparison)
  if (hasPsi && mobileBad && !desktopBad) {
    exec.framing.lines.push(host + " is constrained primarily by mobile delivery in this scan.");
    if (mLCP != null) exec.framing.lines.push("Mobile Largest Contentful Paint (LCP) is " + fmtMs(mLCP) + ", indicating slow visual readiness on phones.");
    else if (mFCP != null) exec.framing.lines.push("Mobile First Contentful Paint (FCP) is " + fmtMs(mFCP) + ", indicating delayed first render on phones.");
    else if (mTTFB != null) exec.framing.lines.push("Mobile Time to First Byte (TTFB) is " + fmtMs(mTTFB) + ", indicating server response delay.");
    if (splitIsStory && dLCP != null) exec.framing.lines.push("Desktop reaches content much faster (LCP " + fmtMs(dLCP) + "), so the constraint is concentrated on mobile.");
  }
  // 2) Slow across devices
  else if (hasPsi && mobileBad && desktopBad) {
    exec.framing.lines.push(host + " is slow to reach a usable state across both mobile and desktop in this scan.");
    if (mLCP != null && dLCP != null) exec.framing.lines.push("LCP is " + fmtMs(mLCP) + " (mobile) and " + fmtMs(dLCP) + " (desktop), indicating delayed primary content rendering.");
  }
  // 3) Desktop-only constraint (rare)
  else if (hasPsi && !mobileBad && desktopBad) {
    exec.framing.lines.push(host + " is constrained primarily by desktop delivery in this scan.");
    if (dLCP != null) exec.framing.lines.push("Desktop Largest Contentful Paint (LCP) is " + fmtMs(dLCP) + ", indicating delayed primary content rendering on desktop.");
  }
  // 4) Trust posture / security headers are the headline
  else if (secMissingCount != null && secMissingCount >= 2) {
    exec.framing.lines.push(host + " has trust hardening gaps that reduce its baseline security posture.");
    exec.framing.lines.push("Security headers/policies are missing (" + secMissingCount + " observed), which can weaken browser protections and user trust signals.");
  }
  // 5) SEO foundation gaps
  else if (seoSig && Array.isArray(seoSig.issues) && seoSig.issues.length) {
    exec.framing.lines.push(host + " has SEO foundation gaps that can reduce crawl clarity and snippet quality.");
    var firstSeo = seoSig.issues[0] || {};
    if (firstSeo.title || firstSeo.id) exec.framing.lines.push("Primary SEO issue: " + String(firstSeo.title || firstSeo.id) + ".");
  }
  // 6) Accessibility gaps
  else if (accessSig && Array.isArray(accessSig.issues) && accessSig.issues.length) {
    exec.framing.lines.push(host + " has accessibility gaps that may affect usability and compliance expectations.");
    var firstA11y = accessSig.issues[0] || {};
    if (firstA11y.title || firstA11y.id) exec.framing.lines.push("Primary accessibility issue: " + String(firstA11y.title || firstA11y.id) + ".");
  }
  // 7) Default (balanced)
  else {
    exec.framing.lines.push(host + " delivery is broadly stable in this scan, with a small number of deterministic issues influencing the score.");
    if (hasPsi && mLCP != null) exec.framing.lines.push("Mobile LCP is " + fmtMs(mLCP) + (dLCP != null ? " vs " + fmtMs(dLCP) + " on desktop." : "."));
  }
