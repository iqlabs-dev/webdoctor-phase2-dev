...
export async function handler(event) {
  try {
    ...
    const reportId = textOrEmpty(event.queryStringParameters?.report_id || event.queryStringParameters?.reportId);
    if (!reportId) return json(400, { success: false, error: "Missing report_id" });

    const { data, error } = await supabase
      .from("scan_results")
      .select("agency_commentary_title, agency_commentary_body, agency_commentary_signoff")
      .eq("user_id", user.id)
