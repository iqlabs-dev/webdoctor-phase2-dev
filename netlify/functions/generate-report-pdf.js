...
// Generates a PDF via DocRaptor using server-rendered HTML (NO JS)
// IMPORTANT:
// - Avoids calling get-report-html-pdf to prevent timeout chains.
// - Fetches JSON once from get-report-data-pdf, renders HTML here, then sends to DocRaptor.

const DOCRAPTOR_API_KEY =
  process.env.DOC_RAPTOR_API_KEY || process.env.DOCRAPTOR_API_KEY || ""; // support both
const DOCRAPTOR_TEST = (process.env.DOCRAPTOR_TEST || "false").toLowerCase() === "true";

// Keep under Netlify gateway limits
const DATA_FETCH_TIMEOUT_MS = 9000;    // fetch scan JSON fast
const DOCRAPTOR_TIMEOUT_MS = 16000;    // docraptor fast fail
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!DOCRAPTOR_API_KEY) {
      return json(500, {
        error: "Missing DOC_RAPTOR_API_KEY in Netlify environment",
        hint: "Set DOC_RAPTOR_API_KEY in Netlify → Project configuration → Environment variables",
      });
    }

    // report_id from GET query or POST body
    let reportId = "";
    ...
    // 1) Fetch JSON (single hop)
    const dataUrl =
      `${siteUrl}/.netlify/functions/get-report-data-pdf?report_id=${encodeURIComponent(reportId)}`;

    const rawText = await fetchWithTimeout(dataUrl, DATA_FETCH_TIMEOUT_MS, {
      method: "GET",
      headers: { Accept: "application/json" },
    }).then(async (r) => {
      const t = await r.text().catch(() => "");
      if (!r.ok) throw new Error(`Report data fetch failed (${r.status}): ${t.slice(0, 300)}`);
      return t;
    });
    ...
    try {
      ...
      return json(500, { error: "Report data endpoint returned non-JSON", details: rawText.slice(0, 600) });
    }

    // 2) Render full HTML here (same structure as get-report-html-pdf)
    const html = renderPdfHtml(jsonData, reportId);

    if (!html || html.length < 500) {
      return json(500, { error: "Rendered HTML is empty/too short", len: (html || "").length });
    }
    ...
    const docReq = {
      ...
      // Prince options can help pagination handling
    ...
    // If DocRaptor returned JSON, that's an error response
    if (contentType.includes("application/json")) {
      const t = await pdfResp.text().catch(() => "");
      return json(502, { error: "DocRaptor returned JSON instead of PDF", details: t.slice(0, 1200) });
    }
...
/* ---------------- PDF HTML renderer (inline) ---------------- */

function renderPdfHtml(json, reportId) {
  ...
  function fixLabel(key) {
    switch (key) {
      case "security": return "Security headers + policy baselines (CSP, X-Frame-Options, Permissions-Policy).";
      case "seo": return "SEO foundations (H1 presence, robots meta, canonical consistency).";
      case "accessibility": return "Accessibility quick wins (empty links/buttons, labels, focus targets).";
      case "performance": return "Performance stability (reduce payload bloat; tame inline script count).";
      case "structure": return "Structure + semantics (document structure and markup clarity).";
      case "mobile": return "Mobile experience validation (already strong — maintain, re-test after changes).";
      default: return "";
    }
  }
...
<body>
  ...
  ${deliverySignalsHtml ? `<h2>Delivery Signals</h2><div class="cards">${deliverySignalsHtml}</div>` : `<h2>Delivery Signals</h2><p class="muted">No delivery signals were available for this report.</p>`}
