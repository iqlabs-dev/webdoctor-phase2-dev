...
const COPY = {
  ...
  mobile_inline_scripts: {
    ...
    client_impact:
      "{count} inline scripts block early rendering, so the site feels slow even on a good connection.",
    tech_label: "Many inline scripts increase mobile render work",
  },
  perf_inline_scripts: {
    client_title: "Too much code runs before the page can display",
    client_impact:
      "{count} inline scripts add render work, so visitors wait longer before seeing content.",
  ...
  perf_head_scripts: {
    ...
    client_impact:
      "Code in the document head runs before anything shows, so the site feels sluggish on first load.",
  ...
  perf_mobile_tbt_high: {
    ...
    client_impact:
      "Heavy JavaScript keeps the page busy (~{tbt}ms blocking time), so taps and scrolls feel laggy.",
  ...
  perf_desktop_tbt_high: {
    ...
    client_impact:
      "Heavy JavaScript keeps the page busy, so interactions feel delayed after load.",
