...
function updateLatestScanCard(row) {
  ...
  // IMPORTANT: Your table uses report_url (not pdf_url)
...
// -----------------------------
// HISTORY LOAD (USE SUPABASE CLIENT - avoids REST auth/RLS surprises)
// -----------------------------
async function loadScanHistory() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      // Date (your UI shows separate date/time columns, but your data is one timestamp)
      const d = row.created_at ? new Date(row.created_at) : null;
      ...
      // If you have a report_url, treat that as the “PDF / report link”
      if (row.report_url) {
